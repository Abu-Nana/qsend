		        ...
		        ";
		         if(mysqli_num_rows(mysqli_query($connection,$check)) == 0)  {
						...
						$fullName = $username . '-' . $firstName . ' ' . $lastName;
					 	//$user = isset($_SERVER['REMOTE_USER']) ? $_SERVER['REMOTE_USER'] : 'Unknown User';
        				$ipAddress = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 'Unknown IP';

						$store = "INSERT INTO files (study_center,student_center_name,exam_session,exam_day,password,file_name,sem,sentby,ip_address) values(
						           '$study_center' ,'$center','$exam_session','$exam_day','$password','$zipfile','241','$fullName', '$ipAddress'
						)";
