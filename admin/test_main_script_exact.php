<?php
/**
 * Test the exact main script logic
 */
...
// Create log file
$log_file = 'email_exact_test_' . date('Y-m-d_H-i-s') . '.log';
...
try {
    writeLog("=== EXACT MAIN SCRIPT TEST STARTED ===");
    ...
    foreach ($centers as $center => $value) {
        ...
        try {
            ...
            // Test PDF processing
            writeLog("Testing PDF processing...");
            $source_file = "DEASemester/" . $value[0]['course'] . ".pdf";
            $destination_file = $folder_path . "/" . $value[0]['course'] . ".pdf";
            
            writeLog("Source file: $source_file");
            writeLog("Destination file: $destination_file");

            if (file_exists($source_file)) {
                copy($source_file, $destination_file);
                writeLog("Copied file: $source_file to $destination_file");
                
                // Test FPDI processing
                if ($vendor_loaded && class_exists('setasign\Fpdi\Fpdi')) {
                    try {
                        ...
                        writeLog("✅ FPDI processing successful");
                    } catch (Exception $pdf_error) {
                        writeLog("❌ FPDI processing failed: " . $pdf_error->getMessage());
                    }
                }
            } else {
                writeLog("❌ Source PDF not found: $source_file");
            }

            writeLog("✅ Basic processing completed for center $center");
            
        } catch (Exception $e) {
            writeLog("❌ Processing failed for center $center: " . $e->getMessage());
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Exact main script test completed successfully',
