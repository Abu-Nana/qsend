...
$query = "select student_registrations_stage.exam_session from `student_registrations_stage` group by `student_registrations_stage`.`exam_session`";
$query_data = mysqli_query($connection,$query);

$day = "select student_registrations_stage.id,student_registrations_stage.exam_day from `student_registrations_stage` group by `student_registrations_stage`.`exam_day` order by `id` asc";
...
?>
	...
	<?php
	 ...
	 ?>
			<form method="POST" action="with_code.php">
            <!--!begin batching
