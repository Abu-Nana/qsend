...
//	$batch = $_POST['batch'];
	...
	if(!isset($exam_session)){
		...
		$script = "select student_registrations.*,study_centers_b1.study_center_code, study_centers_b1.study_centre_email,study_centers_b1.phone_number from `student_registrations` inner join `study_centers_b1` on `study_centers_b1`.`study_center_code` = `student_registrations`.`study_center_code` where `exam_session` = '".$exam_session ."' 
		and `exam_day` = '".$day ."' and `study_centers_b1`.batch = 1 group by `study_centers_b1`.`study_center_code`, `student_registrations`.`course`";
