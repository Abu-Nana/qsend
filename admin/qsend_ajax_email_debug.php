<?php
/**
 * Email Debug AJAX Handler - Enhanced email debugging
 */
    ...
    foreach ($centers as $center => $value) {
        ...
        try {
            ...
            if ($zip->open($zipfile, ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
                ...
                if (!empty($email)) {
                    $email_result = sendEmailWithDebug($email, $zipfile, $password, $subject, $body, $study_center);
 ...
 * Send email with enhanced debugging
 */
function sendEmailWithDebug($email, $attachment, $password, $subject, $body, $study_center) {
    $debug_info = [];
    
    try {
        // Method 1: Try basic mail function first
        $debug_info[] = "Attempting basic mail function...";
...
Directorate of Examinations & Assessment
        ...
        if (mail($email, $subject, $message, $headers)) {
            $debug_info[] = "Basic mail function succeeded";
            return ['success' => true, 'error' => '', 'debug' => implode(' | ', $debug_info)];
        } else {
            $debug_info[] = "Basic mail function failed";
        }
        
        // Method 2: Try PHPMailer with local mail
        $debug_info[] = "Attempting PHPMailer with local mail...";
        ...
        if (file_exists($attachment)) {
            ...
            $debug_info[] = "Attachment added: " . basename($attachment);
        } else {
            $debug_info[] = "Attachment not found: " . $attachment;
        }
        ...
        $mail->send();
        $debug_info[] = "PHPMailer local mail succeeded";
        return ['success' => true, 'error' => '', 'debug' => implode(' | ', $debug_info)];
        
    } catch (Exception $e) {
        $debug_info[] = "PHPMailer error: " . $e->getMessage();
        return ['success' => false, 'error' => $e->getMessage(), 'debug' => implode(' | ', $debug_info)];
    }
}
?>
