...
try{
	...
	$script = "select student_registrations.*,study_centers2.study_center_code, study_centers2.study_centre_email,study_centers2.phone_number from `student_registrations` inner join `study_centers2` on `study_centers2`.`study_center_code` = `student_registrations`.`study_center_code` where `exam_session` = '".$exam_session ."' 
		and `exam_day` = '".$day ."' group by `study_centers2`.`study_center_code`, `student_registrations`.`course`";
		...
		foreach ($centers as $center => $value) {
			...
			$zipfile = "2022_2_compressed_test/".$file_name.".zip";
			$password = "dea".rand(1,1000000);
			$status = false;

			if($zip->open($zipfile, ZipArchive::CREATE  | ZipArchive::OVERWRITE) === TRUE){
				$zip->setPassword($password);
				$paginatedResults = array_chunk($value, 3, true);
		        ...
		        ";
		         if(mysqli_num_rows(mysqli_query($connection,$check)) == 0)  {
						$store = "INSERT INTO files2 (study_center,student_center_name,exam_session	,exam_day,password,file_name,sem) values(
...
function securityCode($dir,$object,$security_code,$connection){
	...
	for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
		...
		if($pageNo == 1){
			...
			$pdf->SetFontSize('5'); // set font size
