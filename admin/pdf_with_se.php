...
try{
	...
	$script = "select student_registrations.*,south_east.study_center_code, south_east.study_centre_email,south_east.phone_number from `student_registrations` inner join `south_east` on `south_east`.`study_center_code` = `student_registrations`.`study_center_code` where `exam_session` = '".$exam_session ."' 
		and `exam_day` = '".$day ."' group by `south_east`.`study_center_code`, `student_registrations`.`course`";
